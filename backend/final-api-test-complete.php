...
echo "<h1>🧪 Complete API Test - All Systems</h1>";
echo "<p>Testing all APIs with enhanced error handling for: <strong>$userEmail</strong></p>";
...
foreach ($apis as $name => $endpoint) {
    ...
    try {
        ...
        echo "<p><small>URL: $url</small></p>";
        
        $response = @file_get_contents($url, false, $context);
        
        if ($response === false) {
            $error = error_get_last();
            $results[$name] = ['status' => 'error', 'message' => 'Network error: ' . ($error['message'] ?? 'Connection failed')];
            echo "<p style='color: red;'>❌ $name: Network error</p>";
            echo "<p><small>Error: " . ($error['message'] ?? 'Connection failed') . "</small></p>";
            continue;
        }
        ...
        if (!$data) {
            ...
            echo "<details><summary>Raw Response</summary><pre>" . htmlspecialchars(substr($response, 0, 500)) . "</pre></details>";
            continue;
        }
        ...
        if ($data['status'] === 'success') {
            ...
            switch ($name) {
                case 'Profile':
                    ...
                    echo "<li>Plan: " . ($sub['plan_code'] ?? 'N/A') . " (" . ($sub['subscription_status'] ?? 'N/A') . ")</li>";
                    echo "<li>Is Pro User: " . (($stats['is_pro_user'] ?? false) ? 'YES' : 'NO') . "</li>";
                    echo "</ul>";
                    break;
                ...
                case 'Subscription Status':
                    ...
                    echo "<ul>";
                    echo "<li>Plan: " . ($data['plan_code'] ?? 'N/A') . "</li>";
                ...
                case 'Live Workshops':
                    ...
                    echo "<ul>";
                    echo "<li>Access Level: " . ($data['access_level'] ?? 'N/A') . "</li>";
                    echo "<li>Total Workshops: " . count($workshops) . "</li>";
                    echo "<li>Message: " . ($data['message'] ?? 'N/A') . "</li>";
                    if (!empty($workshops)) {
                        echo "<li>Sample Workshop: " . ($workshops[0]['title'] ?? 'N/A') . "</li>";
                    }
                    echo "</ul>";
                    break;
            ...
            echo "<p style='color: red;'>❌ $name: " . ($data['message'] ?? 'Unknown error') . "</p>";
            if (isset($data['error_code'])) {
                echo "<p><small>Error Code: {$data['error_code']}</small></p>";
            }
            if (isset($data['debug_info'])) {
                echo "<details><summary>Debug Info</summary><pre>" . htmlspecialchars($data['debug_info']) . "</pre></details>";
            }
        }
...
if ($percentage === 100) {
    ...
    echo "<li>✅ Tutorials API shows full access</li>";
    echo "<li>✅ Notifications API is responsive</li>";
    ...
    echo "<ol>";
    echo "<li><strong>Run the frontend authentication fix</strong> in your browser console (F12)</li>";
    ...
    foreach ($results as $name => $result) {
        if ($result['status'] !== 'success') {
            echo "<li>❌ $name: " . ($result['message'] ?? 'Unknown error') . "</li>";
        }
    }
    echo "</ul>";
    ...
    echo "<ul>";
    echo "<li>Check if your local server is running</li>";
    echo "<li>Verify the database connection</li>";
...
echo "<p><strong>Run this in your browser console (F12) to complete the fix:</strong></p>";
echo "<pre style='background: #000; color: #0f0; padding: 10px; border-radius: 3px; font-size: 12px;'>";
echo "const authData = {\n";
